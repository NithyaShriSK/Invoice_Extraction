...
def smart_correct_invoice(text):



    """Dynamic extractor: No hardcoded industry names."""
    ...
    if not text or not str(text).strip():
        ...
        return json.dumps({"error": "LLM Failure: Empty OCR input"})
    ...
    system_prompt = (
        ...
        "You are an expert Invoice Parser. I will provide a Global OCR and Detailed Slices.\n"

        "RULES:\n"

        "1. MATH: Calculate Qty * Rate. If slice says 34,512 but (6.332 * 5450) is 34509.4, "

        "trust the math and the 'Rupees in words' section (Thirty four thousand...).\n"

        "2. GST: Indian GSTINs start with state codes. Correct '3SEAZ...' to '33EAZ...' based on State Code 33.\n"
    ...
    print("--- Starting Smart Correction (Qwen 2.5:7b) ---")
    ...
    try:
        ...
        response = ollama.generate(
            ...
            prompt=f"PROCESS THIS OCR TEXT:\n{text}",
...
global_text = clean_deepseek_output(global_text)
...
for i, crop in enumerate(crops):
    ...
    temp_path = os.path.join(output_path, f"slice_{i}.png")
    ...
    clean_res = clean_deepseek_output(str(res))
    ...
    if len(clean_res) > 5:
        ...
        print(f"  ✓ Slice {i} captured {len(clean_res)} characters.")

    else:

        print(f"  Slice {i} was too short or empty.")



# 3. Combine for the best prompt context.

combined_ocr = f"GLOBAL LAYOUT:\n{global_text}\n\nDETAILED SLICES:\n" + "\n".join(ocr_outputs)
...
if raw_ocr_text:
    ...
    try:
        ...
        llm_json_str = smart_correct_invoice(raw_ocr_text)
    ...
    print("LLM Correction skipped: OCR result is empty")